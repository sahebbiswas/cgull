...
python3 benchmarks/run_juliet.py --ci

# Filter by specific CWE or category
python3 benchmarks/run_juliet.py --cwe CWE-476
python3 benchmarks/run_juliet.py --category if/else

# Reproduce an individual Juliet test ID
python3 benchmarks/run_juliet.py --test-id CWE476_NULL_Pointer_Dereference__01_baseline

# Export markdown or JSON formatted reports
python3 benchmarks/run_juliet.py --format markdown --output juliet_report.md
```
...
- **Pattern-based rules lack general interprocedural taint tracking**: Rules operating per-line/per-statement (such as `BannedFunctionsRule` CGULL-001 and `FormatStringRule` CGULL-002) flag syntactically risky sinks (like `strcpy(data, src)` or `printf(data)`) based on local statement shape. The CFG has limited intra-file summaries for memory facts, but does not yet propagate general provenance, taint, or bounds across function boundaries. As a result, these rules fire on synthetic benchmark suites (such as Juliet's `GoodSource/BadSink` or `BadSource/GoodSink` variants) whenever the sink syntax is unchanged, even if the buffer or format string variable originates from a safe literal or adequately-sized buffer in the "good" variant. See the [interprocedural analysis milestone](docs/interprocedural-analysis.md) for the proposed scope and rollout plan.
