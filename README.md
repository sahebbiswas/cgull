...
- **Pattern-based rules lack interprocedural taint tracking**: Rules operating per-line/per-statement (such as `BannedFunctionsRule` CGULL-001 and `FormatStringRule` CGULL-002) flag syntactically risky sinks (like `strcpy(data, src)` or `printf(data)`) based on local statement shape. Because they do not perform interprocedural dataflow analysis or track taint across function boundaries, they fire on synthetic benchmark suites (such as Juliet's `GoodSource/BadSink` or `BadSource/GoodSink` variants) whenever the sink syntax is unchanged, even if the buffer or format string variable originates from a safe literal or adequately-sized buffer in the "good" variant.
