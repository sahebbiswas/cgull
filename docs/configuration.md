...
Supported values are `file` and `tu`. CLI selection uses this precedence:

1. explicit `--mode file|tu`;
2. a mode in the discovered or explicitly selected configuration;
3. target-based inference.

When mode is inferred, any effective directory target selects `tu`; a target list containing only files selects `file`. The no-target `cgull scan` form is equivalent to scanning `.` and therefore selects `tu`. Mixed file/directory target lists also select `tu`.

This makes mode optional in normal project configuration. For example, a `.cgull.toml` generated by `cgull init` can leave mode unset so project-directory scans receive TU analysis while direct single-file scans retain lightweight file mode. Configure a fixed mode only when the repository wants to override that natural target-based behavior for every invocation.

An explicit CLI value always wins, so both directions remain available:

```bash
cgull scan . --mode file
cgull scan src/example.c --mode tu
```

The CLI reports the selected mode and its source (`command line`, `configuration`, or `inferred from targets`) in human-readable output. JSON metadata uses `meta.scan_mode` and `meta.scan_mode_source`; SARIF invocation properties use `scanMode` and `scanModeSource`.

Target inference is intentionally limited to the CLI. Programmatic callers using `ScanConfig.create()` retain the backward-compatible `ScanMode.FILE` default unless they choose another mode explicitly. TU mode expands resolvable project headers and carries source provenance back to original files; see [Analysis model](analysis-model.md).
