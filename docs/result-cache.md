...
cgull scan . --cache-path "$RUNNER_TEMP/cgull-cache"
...
`--cache-dir` is a boolean flag (it never consumes the scan target). Pass an
explicit directory with `--cache-path PATH` (or `CGULL_CACHE_DIR`).
...
Changing any of those inputs yields a miss and recomputes the file. On a hit,
path-dependent Issue fields and fingerprints are also rewritten for the
requesting path as defense in depth.
...
- Unrepresentable configurations (fingerprint construction failures) bypass the
  cache and continue with a normal scan rather than failing.

## Verification tips

Warm-cache behavior can be checked by scanning twice with the same `--cache-dir`
or `--cache-path` and confirming the second run finishes much faster on an
unchanged tree while producing the same findings/fingerprints. Injecting a
truncated JSON file under the cache directory should force a miss rather than a
scan failure.
