...
class StructuredCFG(LegacyDataflowMixin, StructuredGraph):
    ...
    def _degrade_unknown_control_flow_facts(self) -> None:
        ...
        An ``unknown_control_flow`` event represents a direct goto whose label
        could not be resolved.  Construction gives that event wildcard graph
        successors.  Any node reachable through the wildcard must therefore be
        treated as potentially preceded by omitted/unknown execution.
        """
        unknown_ids = {
            node_id
            for node_id, node in self.nodes.items()
            if node.is_unknown_control_flow
        }
