"""Legacy nullness, initialization, allocation, and location-state transfer.

The mixin operates on the graph contract supplied by :class:`StructuredGraph`.
...
class LegacyDataflowMixin:
    def analyze_dataflow(
        self,
        initial_nonnull: Optional[Set[str]] = None,
        initial_initialized: Optional[Set[str]] = None,
        all_vars: Optional[Set[str]] = None,
    ) -> None:
        """Run fixed-point nullness, initialization, and allocation analysis."""
        if not self.blocks:
        ...
        self.realloc_records: Dict[
            str, Tuple[str, str, Set[str], Dict[str, Allocation], bool]
        ] = {}
        ...
        entry_block_id = (
            self.node_to_block.get(self.entry)
            if self.entry
            else min(self.blocks.keys())
        )
        ...
        if entry_block:
            for v in all_vars:
                entry_block.nullness_in[v] = (
                    Nullness.NON_NULL if v in init_nonnull else Nullness.UNKNOWN
                )
                entry_block.init_in[v] = (
                    Initialization.INITIALIZED
                    if v in init_initialized
                    else Initialization.UNINITIALIZED
                )
        ...
        while worklist:
            ...
            curr_loc_map = {
                v: set(locs) for v, locs in block.loc_map_in.items()
            }
            ...
            for node in block.nodes:
                if getattr(node, "realloc_bindings", None):
                    for target_var, input_ptr in node.realloc_bindings.items():
                        ...
                        pre_states = {
                            loc: curr_loc_state.get(
                                loc, Allocation.NOT_ALLOCATED
                            )
                            for loc in input_locs
                        }
                        base_loc_id = f"alloc_{node.node_id}_{target_var}"
                        new_loc_id = base_loc_id
                        if curr_loc_state.get(base_loc_id) in (
                            Allocation.FREED,
                            Allocation.MAYBE_FREED,
                        ) or any(
                            other != target_var
                            and base_loc_id in curr_loc_map.get(other, set())
                        ...
                        if ast_node is not None:
                            val_call = _find_value_producing_call(
                                getattr(ast_node, "init", None)
                                or getattr(ast_node, "rvalue", None)
                            )
                        ...
                        while v_queue:
                            ...
                            for succ_b_id in self.blocks[curr_v].successors:
                                ...
                                if (
                                    succ_b
                                    and len(succ_b.predecessors) == 1
                                    and succ_b_id not in valid_blocks
                                ):
                                    valid_blocks.add(succ_b_id)
                                    v_queue.append(succ_b_id)

                        self.realloc_records[new_loc_id] = (
                            target_var,
                            input_ptr,
                            input_locs,
                            pre_states,
                ...
                for ptr in node.realloc_inputs:
                    ...
                    for loc in locs:
                        curr_loc_state[loc] = meet_allocation(
                            curr_loc_state.get(loc, Allocation.NOT_ALLOCATED),
                            Allocation.MAYBE_FREED,
                        )
                ...
                for v in node.writes:
                    ...
                    if v not in node.allocated:
                        if v in node.alias_writes:
                            ...
                            curr_null[v] = curr_null.get(
                                rhs_var, Nullness.UNKNOWN
                            )
                        else:
            ...
            for v in all_vars:
                ...
                if not locs:
                    ...
                    for loc in locs:
                        loc_st = curr_loc_state.get(
                            loc, Allocation.NOT_ALLOCATED
                        )
                        v_alloc = (
                            loc_st
                            if v_alloc is None
                            else meet_allocation(v_alloc, loc_st)
                        )
                    curr_alloc[v] = (
                        v_alloc
                        if v_alloc is not None
                        else Allocation.NOT_ALLOCATED
                    )
            ...
            for succ_id in block.successors:
                ...
                edge_fact = block.edge_facts.get(
                    succ_id, (set(), set())
                )
                ...
                for v in all_vars:
                    ...
                    if v not in succ_block.nullness_in:
                        ...
                        new_null = meet_nullness(
                            succ_block.nullness_in[v], e_null
                        )
                    if succ_block.nullness_in.get(v) != new_null:
                        succ_block.nullness_in[v] = new_null
                        changed = True

                    e_init = curr_init.get(
                        v, Initialization.UNINITIALIZED
                    )
                    if v not in succ_block.init_in:
                        new_init = e_init
                    else:
                        new_init = meet_initialization(
                            succ_block.init_in[v], e_init
                        )
                ...
                for new_loc_id, rec in self.realloc_records.items():
                    ...
                    if (
                        ...
                        and block.block_id not in valid_blocks
                    ):
                        continue

                    if new_loc_id in curr_loc_map.get(target_var, set()):
                        if curr_null.get(
                            target_var, Nullness.UNKNOWN
                        ) in (Nullness.MAYBE_NULL, Nullness.UNKNOWN):
                            t_null = edge_null.get(
                                target_var, Nullness.UNKNOWN
                            )
                            if t_null == Nullness.NON_NULL:
                                for loc in input_locs:
                                    if pre_states.get(loc) in (
                                ...
                                for loc in input_locs:
                                    if size_is_zero:
                                        ...
                                        edge_loc_state[loc] = pre_states.get(
                                            loc, Allocation.NOT_ALLOCATED
                                        )
                                edge_loc_state[new_loc_id] = Allocation.NOT_ALLOCATED

                all_loc_ids = set(edge_loc_state.keys()).union(
                    succ_block.loc_state_in.keys()
                )
                for loc in all_loc_ids:
                    e_loc_st = edge_loc_state.get(
                        loc, Allocation.NOT_ALLOCATED
                    )
                    if loc not in succ_block.loc_state_in:
                        new_loc_st = e_loc_st
                    else:
                        new_loc_st = meet_allocation(
                            succ_block.loc_state_in[loc], e_loc_st
                        )
                ...
                for v in all_vars:
                    ...
                    if not locs:
                        ...
                        v_alloc = None
                        for loc in locs:
                            l_st = succ_block.loc_state_in.get(
                                loc, Allocation.NOT_ALLOCATED
                            )
                            v_alloc = (
                                l_st
                                if v_alloc is None
                                else meet_allocation(v_alloc, l_st)
                            )
                        new_alloc = (
                            v_alloc
                            if v_alloc is not None
            ...
            for node in block.nodes:
                ...
                self.node_facts[node.node_id] = {
                    v: VariableFacts(
                        ...
                        initialization=curr_init.get(
                            v, Initialization.UNINITIALIZED
                        ),
                        allocation=curr_node_alloc.get(
                            v, Allocation.NOT_ALLOCATED
                        ),
                    )
                    for v in all_vars
                }
    ...
    def query_initialization(
        self, var_name: str, node_id: int
    ) -> Initialization:
        facts = self.get_facts_at_node(node_id)
        if var_name in facts:
