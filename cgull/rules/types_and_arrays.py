...
class PointerSubtractionSizeRule(BaseRule):
    ...
    def scan_ast(self, file_path: str, ast_ctx: CASTContext) -> List[Issue]:
        ...
        def _is_unscaled_pointer_subtraction(node, fn) -> bool:
            # We are looking for (ptrA - ptrB) where ptrA/ptrB are not char*
            if isinstance(node, c_ast.BinaryOp) and node.op == '-':

                # Check if it's scaled by sizeof or a literal inside another expression (e.g., (p2 - p1) * sizeof(...))
                # If we were passed the binary op `-`, it means this is the top-level expression for the argument.
                # If the size argument itself is a subtraction, check its operands.
                    ...
                    if isinstance(n, c_ast.ID):
                        ...
                        if '*' in type_str:
                            if not is_byte_ptr(type_str):
                             ...
                             # Cast to non-pointer, probably an integer (like ptrdiff_t). If they cast to int before subtract,
                             # it's just int subtraction, not pointer subtraction in C terms. But if they cast a pointer subtraction to int,
                             # the subtraction itself is still pointer subtraction.
                             pass
                    elif isinstance(n, c_ast.UnaryOp) and n.op == '&':
                         type_str = _format_pycparser_expr(n)
                         if not is_byte_ptr("void*", type_str): # Hack to just check if the resulting pointer is not byte ptr
                             # Wait, '&' on an int gives int*, which is a non-byte pointer.
                             # Let's check the type of the operand... it's hard without full type inference.
                             # Assume &var gives a non-byte pointer unless var is char.
                             return True
                ...
                if left_is_ptr and right_is_ptr:
                 ...
                 # Check the inner expression of the cast
...
class PointerSubtractionSizeRule(BaseRule):
    ...
    def scan_ast(self, file_path: str, ast_ctx: CASTContext) -> List[Issue]:
        ...
        from pycparser import c_ast
        from ..ast_analyzer import _format_pycparser_expr, find_function_def
